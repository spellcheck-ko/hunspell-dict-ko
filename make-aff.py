   ...
   { 'id': 14,
    ...
    { 'id': 24,
      ...
      'cond': ['[%s]' % all_vowel_ao.replace(u'\u1165',''), # 'ㅓ' 제외
               '[%s][%s]' % (all_vowel_ao, all_trailing)],
      'after': ['#용언'],
      'except': ['#ㅂ불규칙', '#ㅅ불규칙', '#ㅎ불규칙', '#르불규칙', '#으불규칙'],
    },
    # ㅓ 뒤에 -어: ㅓ 탈락
    { 'id': 24,
...
#    },
    ...
    # NOTE: '으' 음절 앞에 오는 음절이 양성모음이냐 음성모음이냐에 따라 어미의
    # '어/아'가 결정되는데, '크다', '쓰다', '끄다'의 경우 앞의 음절이 없으면서
    # '어'가 붙는다. 그래서 aff 파일의 같은 규칙 안에서 조건으로 정의할 수가 없다.
    # 그러므로 '크다', '쓰다', '끄다'는 항상 별도 규칙으로 만든다.

    # 았 앞에서 으 탈락
    { 'id': 140, 'groupid': 5,
      'name': u'-\u1161\u11bb-',
      'cond': [ u'[\u1161\u1169][%s]\u1173' % all_leading,
                u'[\u1161\u1169][%s][%s]\u1173' % (all_trailing, all_leading) ],
      'strip': u'\u1173',
      'after': ['#으불규칙'],
    },
    # 었 앞에서 으 탈락
    { 'id': 140, 'groupid': 5,
      'name': u'-\u1165\u11bb-',
      'cond': [ u'[%s][%s]\u1173' % (all_vowel_ao, all_leading),
                u'[%s][%s][%s]\u1173' % (all_vowel_ao, all_trailing, all_leading) ],
      'strip': u'\u1173',
      'after': ['#으불규칙'],
    },
    # 크다/쓰다/끄다
