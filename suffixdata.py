...
groups[u'-었-'] = [
    { 'rules': [[u'-었-', COND_EOA_NOT_AO, ''],
      ...
      'after': ['#용언', '#이다', '-으시-'],
      'notcond': ['#ㅂ불규칙', '#ㅅ불규칙', '#르불규칙', '#ㅎ불규칙', '#으불규칙'],
    },
    # ㅂ불규칙
...
groups[u'-어'] = [
    { 'rules': [[u'-어', COND_EOA_NOT_AO, ''],
      ...
      'after': ['#용언'],
      'notcond': ['#ㅂ불규칙', '#ㅅ불규칙', '#ㅎ불규칙', '#르불규칙', '#으불규칙'],
    },
    # ㅂ불규칙
    ...
    # ㅅ불규칙
    { 'rules': [[u'-어', '[%s]%s' % (V_A_O, T_SIOS), T_SIOS],
                [u'-아', '[%s]%s' % (V_NOT_A_O, T_SIOS), T_SIOS]],
      'after': ['#용언'],
      'cond': ['#ㅅ불규칙'],
    },
    # ㅎ불규칙
...
### 연결: -어서, -아서
# '-어' 재활용
groups[u'-어서'] = [k.copy() for k in groups[u'-어']]
...
### 연결: -어도, -아도
# '-어' 재활용
groups[u'-어도'] = [k.copy() for k in groups[u'-어']]
for klass in groups[u'-어도']:
    new_rule = []
    for r in klass['rules']:
        new_rule.append([r[0] + '도'] + r[1:])
    klass['rules'] = new_rule

### 연결: -어요, -아요
# '-어' 재활용
groups[u'-어요'] = [k.copy() for k in groups[u'-어']]
...
### 연결: -어야, -아야
# '-어' 재활용
groups[u'-어야'] = [k.copy() for k in groups[u'-어']]
for klass in groups[u'-어야']:
    new_rule = []
    for r in klass['rules']:
        new_rule.append([r[0] + '야'] + r[1:])
    klass['rules'] = new_rule

### 연결: -어다
# '-어' 재활용
groups[u'-어다'] = [k.copy() for k in groups[u'-어']]
...
#### 연결: -ㄹ지라도, -을지라도
groups[u'-을지라도'] = [
    { 'rules': [[u'-\u11af지라도', COND_V_ALL, ''],
                [u'-\u11af지라도', T_RIEUL, T_RIEUL],
                [u'-을지라도', COND_T_NOT_RIEUL, '']],
      'after': ['#용언', '#이다', '-으시-', '-었-'],
      'notcond': ['#ㅂ불규칙', '#ㅅ불규칙'],
    },
]
...
### 연결: -ㄹ까, -을까
groups[u'-을까'] = [
    { 'rules': [[u'-\u11af까', COND_V_ALL, ''],
                [u'-\u11af까', T_RIEUL, T_RIEUL],
                [u'-을까', COND_T_NOT_RIEUL, '']],
      'after': ['#용언', '#이다', '-으시-', '-었-'], 
      'notcond': ['#ㅂ불규칙', '#ㅅ불규칙', '#ㅎ불규칙'],
    },
    # ㅂ불규칙
    PIEUP_IRREGULAR_TYPICAL_CLASS(u'-울까', ['#용언']),
    # ㅅ불규칙
    SIOS_IRREGULAR_TYPICAL_CLASS(u'-을까', ['#용언']),
    # ㅎ불규칙
    { 'rules': [[u'-\u11af까', T_HIEUH, T_HIEUH]],
      'after': ['#용언'],
      'cond': ['#ㅎ불규칙'],
    },
]
...
### 연결: -ㄹ수록, -을수록
groups[u'-을수록'] = [
    { 'rules': [[u'-\u11af수록', COND_V_ALL, ''],
                [u'-\u11af수록', T_RIEUL, T_RIEUL],
                [u'-을수록', COND_T_NOT_RIEUL, '']],
      'after': ['#용언', '#이다', '-으시-', '-었-'],
      'notcond': ['#ㅂ불규칙', '#ㅅ불규칙', '#ㅎ불규칙'],
    },
    # ㅂ불규칙
    { 'rules': [[u'-울수록', T_PIEUP, T_PIEUP]],
      'after': ['#용언'],
      'cond': ['#ㅂ불규칙'],
    },
    # ㅅ불규칙
    { 'rules': [[u'-을수록', T_SIOS, T_SIOS]],
      'after': ['#용언'],
      'cond': ['#ㅅ불규칙'],
    },
    # ㅎ불규칙
    { 'rules': [[u'-\u11af수록', T_HIEUH, T_HIEUH]],
      'after': ['#용언'],
      'cond': ['#ㅎ불규칙'],
    },
]
...
groups[u'-으나'] = [
    ...
    # ㅂ불규칙
    { 'rules': [[u'-우나', T_PIEUP, T_PIEUP]],
      'after': ['#용언'],
      'cond': ['#ㅂ불규칙'],
    },
    # ㅅ불규칙
    { 'rules': [[u'-으나', T_SIOS, T_SIOS]],
      'after': ['#용언'],
      'cond': ['#ㅂ불규칙'],
    },
]
...
groups[u'-읍시다'] = [
    ...
    # ㅂ불규칙
    { 'rules': [[u'-웁시다', T_PIEUP, T_PIEUP]],
      'after': ['#동사'],
      'cond': ['#ㅂ불규칙'],
    },
    # ㅅ불규칙
    { 'rules': [[u'-읍시다', T_SIOS, T_SIOS]],
      'after': ['#동사'],
      'cond': ['#ㅅ불규칙'],
    },
]
...
### 연결: -ㄹ지, -을지
groups[u'-을지'] = [
    { 'rules': [[u'-\u11af지', COND_V_ALL, ''],
                [u'-\u11af지', T_RIEUL, T_RIEUL],
                [u'-을지', COND_T_NOT_RIEUL, '']],
      'after': ['#용언', '#이다', '-으시-', '-었-'],
      'notcond': ['#ㅂ불규칙', '#ㅅ불규칙'],
    },
    # ㅂ불규칙
    PIEUP_IRREGULAR_TYPICAL_CLASS(u'-울지', ['#용언']),
    # ㅅ불규칙
    SIOS_IRREGULAR_TYPICAL_CLASS(u'-을지', ['#용언']),
]
...
groups[u'-으니'] = [
    ...
    # ㅂ불규칙
    PIEUP_IRREGULAR_TYPICAL_CLASS(u'-우니', ['#형용사']),
    # ㅅ불규칙
    SIOS_IRREGULAR_TYPICAL_CLASS(u'-으니', ['#형용사']),
]
