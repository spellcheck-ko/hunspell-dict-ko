...
# 없다. (aff 파일에서 쓸 수 있는 condition으로는 으 앞에 없다는 걸
# 정의할 수가 없다.) 그러므로 '끄다', '뜨다', '쓰다', '크다'는 항상
# 별도 규칙으로 만든다.
...
# 참고: 선어말어미 
# 
# 적은 수의 어말어미와 제한적으로만 결합하는 선어말어미는 (교착적
# 선어말어미) 여기에서 별도의 그룹으로 취급하지 않고, 결합이 자유로운
# 선어말 어미만 (분리적 선어말어미) 취급한다. 예를 들어 '-느냐'에서
# '-느-' 선어말어미를 별도로 취급하지 않는다. 이러한 선어말어미는
# 사전에도 어말어미와 붙인 형태로 기재되어 있고, 가능한 모든 어미의
# 가짓수가 몇 개 되지 않으므로 별도 그룹으로 취급할 필요가 없다.
...
attach_emphasis(groups[u'-으라고'], ['요'])
